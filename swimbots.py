...
class Swimbot:
    def __init__(self, space, start_pos, genes, color):
        ...
        for i in range(SEGMENTS_PER_BOT):
            mass = 0.7  # Zmniejszamy masę z1 do 0.5 (lżejszy bot łatwiej ruszy)
            moment = pymunk.moment_for_box(mass, (segment_width, segment_height))
            body = pymunk.Body(mass, moment)
            body.velocity_limit = 400  # Maksymalna prędkość, jakiej bot może doznać 500
            body.angular_velocity_limit = 15 # Limit obrotów, żeby nie wirował jak szalony
            ...
            if i > 0:
                ...
                spring = pymunk.DampedRotarySpring(
                    ...
                    self.bodies[i], 
                    0,
                    10000, # ZWIĘKSZONO z 3000 na 15000 (silniejsze mięśnie)
...
def mutate(genes):
    ...
    for i, val in enumerate(genes):
        val += random.uniform(-0.1, 0.1)
        # Nakładamy limity na geny:
        low, high = GENE_RANGES[i]
        val = max(low, min(val, high)) # Pilnujemy, by gen nie wyszedł poza zakres
        new_genes.append(val)
    return new_genes

def setup_simulation(genes_list=None):
    space = pymunk.Space()
    space.damping = 0.8  # 0.9 to lekki opór, który pozwala na zachowanie pędu
    ...
    for i in range(POPULATION_SIZE):
        ...
        pos = (start_x, start_y + random.randint(-200, 200))
